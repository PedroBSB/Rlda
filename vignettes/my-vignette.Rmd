...
res<-lda_multinomial(data=mat1,n_community=30,beta,gamma,n_gibbs=1000,ll_prior=TRUE,display_progress=FALSE)
```
...
In Financial Markets, firms are commonly evaluated based on their liquidity (i.e., liquidity of a firm refers to its ability to meet short-term obligations using firm's assets that can be quickly converted to cash [@audretsch2002does]). 

In this example we studied how firms can be clustered based on two main factor: the firms' free float\footnote{Free float is the number of outstanding shares that are available for trading by the public.} and the negotiated volume\footnote{Volume is number of shares that traded in a particular time period.}.

We can approximate the volume random variable for each firm in each day as a Binomial trial, since the volume should be less than free float for the same firm and day \footnote{In fact this is just an approximation, since one share can be negotiated multiple times in the same day.}.

In this example we worked with all daily transactions for 46 firms of the S&P500 Index in 2015, obtained from the Reuters Terminal. The data represented the accepted value of daily transactions for a free float equal to 100:
...
In this case, we use the \code{lda_binomial}, for which we need to specify the follow arguments:
...
res<-lda_binomial(sp500,spSize,10,alpha0,alpha1,gamma,5000,TRUE,FALSE)
```
...
Financial investors can, for example, using the information presented in Figure \ref{figure:fig002} to construct portfolios based on these clusters, aiming to diversify their assets or studied the similar behavior associated with the liquidity clusters.

This is another way to evaluate the results of the package's functions. In the first example we presented the results using only tables of frequency, now in this example we present the results in a more visually way, where each cluster is represented by a color and the size of the bar is associated with the probability of belonging.
...
Phi<-res[["Phi"]]
#Burnout
Phi<-colMeans(Phi[1000:5000,])
#Create the matrix
Phi<-matrix(Phi,nrow = ncol(matPres),ncol=10)
#Rownames
rownames(Phi)<-colnames(matPres)
colnames(Phi)<-paste("Cluster ",seq(1,ncol(Phi)))
#Color
library(RColorBrewer)
myColor<-brewer.pal(n = 10, name = "RdBu")
#Labels
strClusters<-colnames(Phi)
