...
NumericVector matchBinomialCovaResult(double value,NumericVector vec){
  int cont=0;
...
NumericVector matchBinomialCovaIndex(double value,NumericVector vec){
  int cont=0;
  for(int i=0;i<vec.length();i++){
    if(vec(i)!=value){
      cont=cont+1;
...
NumericVector rdirichletBinomialCova(Rcpp::NumericVector parms) {
...
NumericMatrix rdirichletBinomialCova(int n, Rcpp::NumericVector parms) {
...
int whichLessDVBinomialCova(double value, NumericVector prob) {
  int res = -1;
...
NumericVector rmultinomialDVBinomialCova(int size, NumericVector prob) {
  ...
  while(count<size){
    ...
    int iPos=whichLessDVBinomialCova(random,prob);
...
NumericVector invertedCumsumBinomialCova(NumericVector n){
...
NumericVector meltBinomialCova(NumericMatrix mat){
...
void updateThetaAndPhiBinomialCova(NumericMatrix &ThetaGibbs,NumericMatrix Theta,NumericMatrix &PhiGibbs, NumericMatrix Phi,NumericMatrix &BetaGibbs, NumericMatrix Beta, int gibbs){
  //'meltBinomialCova the Theta and Phi matrix
  ThetaGibbs(gibbs,_)=meltBinomialCova(Theta);
  PhiGibbs(gibbs,_) =meltBinomialCova(Phi);
  BetaGibbs(gibbs,_) =meltBinomialCova(Beta);
}

NumericMatrix aggregateValuesBinomialCova(GenericVector zMat,int nLocations,int n_community){
...
List aggregateValuesBinomialCovaByReflectanceBinomialCova(GenericVector zMat,int nBands,int n_community){
...
double sumLargestBinomialCova(NumericMatrix nSum, int iCommunity, int iLocation){
  double sum=0.0;
  //'For each community
...
NumericMatrix mmultBinomialCova(const NumericMatrix& m1, const NumericMatrix& m2){
...
GenericVector generateZBinomialCova(NumericMatrix binomMat,NumericMatrix populMat, NumericMatrix Theta, NumericMatrix Phi) {
  try{
    //'Number of locations
    ...
    for(int b=0;b<nBands;b++){
      ...
      for(int l=0;l<nLocations;l++){
        ...
        if(binomMat(l,b)!=0){
          ...
          //'Store the results
          NumericVector tmp = rmultinomialDVBinomialCova(iSize, prob);

          //'Find the community
          NumericVector iCommunity = matchBinomialCovaIndex(0.0,tmp);
          NumericVector iResults =   matchBinomialCovaResult(0.0,tmp);
...
NumericMatrix generateThetaBinomialCova(GenericVector zMat, NumericMatrix& vMat, int nLocations,int n_community, double gamma) {
  ...
  NumericMatrix sumMat = aggregateValuesBinomialCova(zMat, nLocations, n_community);
  ...
  for(int l=0;l<nLocations;l++){
    ...
    for(int c=0;c<n_community;c++){
      if(c<(n_community-1)){
        ...
        double nLCgreter=sumLargestBinomialCova(sumMat, c, l);
...
NumericMatrix generatePhiBinomialCova(GenericVector zMat,int nBands,int n_community,double alpha0, double alpha1) {
  //'Initialize the Phi matrix
  NumericMatrix Phi(nBands,n_community);
  //'Get the sum matrices
  List sumList = aggregateValuesBinomialCovaByReflectanceBinomialCova(zMat,nBands,n_community);
...
double ll_priorFunctionBinomialCova(NumericMatrix matDATA,NumericMatrix matPOP, NumericMatrix vMat, NumericMatrix Theta, NumericMatrix Phi, double alpha0,double alpha1, double gamma, bool ll_prior=true) {
  ...
  NumericMatrix probs=mmultBinomialCova(Theta,tPhi);
...
//' @name GibbsSamplingBinomialCova
//' @title Compute the Gibbs Sampling for LDA BinomialCova
//' @description Compute the Gibbs Sampling for LDA BinomialCova
//' @param DATA - DataFrame with Presence and Absecence (BinomialCova)
//' @param POP - DataFrame with Population Size (BinomialCova)
...
List lda_binomial_cov(DataFrame data,DataFrame pop, DataFrame design, int n_community, double alpha0, double alpha1, double gamma, int n_gibbs, bool ll_prior=true, bool display_progress=true) {
  ...
  hyperTheta.fill(1);
  NumericMatrix Theta=rdirichletBinomialCova(nLocations,hyperTheta);
  NumericMatrix vMat(nLocations,n_community);

  //'Intialize Phi
  NumericVector hyperPhi(n_community);
  hyperPhi.fill(1);
  NumericMatrix Phi=rdirichletBinomialCova(nBands,hyperPhi);
  ...
  for (int g = 0; g < n_gibbs; ++g) {
    ...
    //'Initialize the zMAt
    GenericVector zMat = generateZBinomialCova(matDATA, matPOP, Theta, Phi);

    //'Generate Theta
    Theta = generateThetaBinomialCova(zMat,vMat, nLocations, n_community, gamma);

    //'Generate Phi
    Phi = generatePhiBinomialCova(zMat, nBands, n_community, alpha0, alpha1);
    ...
    updateThetaAndPhiBinomialCova(ThetaGibbs, Theta, PhiGibbs, Phi,BetaGibbs, betasCoef, g);

    //'Initialize the logLikelihood
    double logLikelihood=ll_priorFunctionBinomialCova(matDATA,matPOP,
                                                  vMat, Theta, Phi,
                                                  alpha0, alpha1, gamma,
                                                  ll_prior);
