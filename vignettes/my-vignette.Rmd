...
abstract: >
  The goal of this paper is to describe the Bayesian LDA model for fuzzy clustering based on different types of data (i.e., Multinomial, Bernoulli, and Binomial entries) and provide some examples of the use of this model in R. These types of data frequently emerge in fields like as disparate as ecology, remote sensing, marketing, and finance and others fields and, as a result, we believe this package will be of broad interest for pattern recognition, especially fuzzy clustering for categorical data.
keywords:
...
These models are fit using Gibbs Sampling where parameters draw are iteratively made from each full conditional distribution. From a conceptual perspective, all of these models assume the following matrix decomposition:
...
\begin{equation}
\begin{array}{lll}
p(Z_{lc}=c^{*}|Y_{ls}=y_{ls},\boldsymbol\phi_{s},\boldsymbol\theta_{l})&=&\frac{\theta_{lc^{*}}\phi_{c^{*}s}^{x_{ils}}(1-\phi_{c^{*}s})^{1-x_{ils}}}{\displaystyle\sum_{c=1}^{C}\theta_{lc}\phi_{cs}^{x_{ils}}(1-\phi_{cs})^{1-x_{ils}}}
\end{array}
\label{eq:eq0006} 
\end{equation}
...
res<-lda_multinomial(mat1,30,beta,gamma,1000,TRUE,FALSE)
```
...
We can visually evaluate the convergence analyzing the Figure \ref{figure:fig001}:
...
In this cases since we are postulated a Binomial trial we should use the \code{lda_binomial}, for which we need to specify the follow arguments:
...
We can also note that for most of the firms, the probability of belonging is very similar and distributed across the clusters, but this not the general case. Usually, when data can be partitioned in a hard way, some clusters will have more probability than others.
...
For this kind of data, we can use the the \code{lda_bernoulli} function. Specifically, in this example we used \code{data("SPDATA")} from \pkg{PresenceAbsence} package which is referred to 13 species that could be observed (value equal 1) and not observed or absent (value equal 0) at 386 forested locations [@moisen2006predicting]. 
...
In this example we can postulated a Bernoulli trial for each Specie and Location, and we are able to use the \code{lda_bernoulli} function, for which we need to specify the follow arguments:
